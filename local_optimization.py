...
class StochasticReconfiguration:
    ...
    def __call__(self, parameters, walkerRs, learningRate, diagonalShift, parameterMask=None):

        if parameterMask is None:
            parameterMask = jax.tree_util.tree_map(lambda p: jnp.ones_like(p), parameters)
        ...
        (flatParameters,unravel) = jax.flatten_util.ravel_pytree(parameters)
        flatParameterMask = jax.flatten_util.ravel_pytree(parameterMask)[0]   # (P,)
        ...
        f_k = f_k * flatParameterMask
        U = U * flatParameterMask[None,:]
        ...
        parameterStep = scale * learningRate * parameterStep * flatParameterMask
        updatedParameters = unravel(flatParameters + parameterStep)
...
class StochasticReconfigurationMomentum:
    ...
    def __call__(self, parameters, walkerRs, learningRate, diagonalShift, mu, history, parameterMask=None):
