...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(5e-1,5e2)
...
class LogMP2SJB(wavefunctions.Wavefunction):
    """
...
upNoise = np.random.normal(size=diagMatrix.shape)
downNoise = np.random.normal(size=diagMatrix.shape)
upCoeffs = jnp.array(diagMatrix) + 1e-3 * upNoise
downCoeffs = jnp.array(diagMatrix) + 1e-3 * downNoise
...
wavefunction = LogMP2SJB(
    spins, dim, lattice, kpoints, upCoeffs, downCoeffs, 32
)
