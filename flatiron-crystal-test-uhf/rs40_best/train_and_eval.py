...
( uhf_energy , testKpoints , spinUpCoeff , spinDownCoeff ) = wavefunctions.uhfInitialization(
    spins, dim, lattice, kpoints
)

assert jnp.linalg.norm(testKpoints - kpoints) < 1e-6

np.save("init_spinUpCoeff.npy", spinUpCoeff)
np.save("init_spinDownCoeff.npy", spinDownCoeff)

print("UHF Reference State Energy: {}".format(uhf_energy / N))
