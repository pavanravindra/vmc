...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(1e-2,5e-1)
...
class TestWavefunction(wavefunctions.Wavefunction):
    """
    Two-round neural-backflow wavefunction.

    Round 1 is identical to the original architecture so that old trained
    parameters can be loaded into the matching parameter names unchanged.

    Round 2 builds a new embedding from pair features constructed out of:
        v_ij,
        h_i^(1) - h_j^(1),
        h_i^(1) * h_j^(1)

    The final round-2 Jastrow and backflow readouts are zero-initialized so
    that, after copying over the trained round-1 parameters, the wavefunction
    at step 0 is exactly the old trained wavefunction.
    """
    spins: (int, int)
    dim: int
    lattice: jnp.ndarray
    ...
    def setup(self):
        ...
        self.weights1_nb_2 = nn.Dense(self.hiddenFeatures)
        self.weights2_nb_2 = nn.Dense(1)

        self.linear1_nb_2 = nn.Dense(self.hiddenFeatures)
        self.linear2_nb_2 = nn.Dense(self.hiddenFeatures)

        self.jastrow1_nb_2 = nn.Dense(self.hiddenFeatures)
        self.jastrow2_nb_2 = nn.Dense(
            1, kernel_init=nn.initializers.zeros, bias_init=nn.initializers.zeros
        )

        self.backflow1_nb_2 = nn.Dense(self.hiddenFeatures)
        self.backflow2_nb_2 = nn.Dense(
            self.dim, kernel_init=nn.initializers.zeros, bias_init=nn.initializers.zeros
        )
    ...
    def __call__(self, rs):
        ...
        h_ij = self.linear2_nb(nn.swish(self.linear1_nb(v_ij))) # (N, N, hiddenFeatures)
        ...
        # ======================
        h_i_row = h_i[:, None, :]   # (N, 1, hiddenFeatures)
        h_i_col = h_i[None, :, :]   # (1, N, hiddenFeatures)

        dh_ij = h_i_row - h_i_col   # (N, N, hiddenFeatures), i - j
        ph_ij = h_i_row * h_i_col   # (N, N, hiddenFeatures), elementwise product

        z_ij = jnp.concatenate(
            [v_ij, dh_ij, ph_ij], axis=-1
        )  # (N, N, 2*dim+2 + 2*hiddenFeatures)

        g_ij_2 = nn.sigmoid(
            self.weights2_nb_2(nn.swish(self.weights1_nb_2(z_ij)))
        )  # (N, N, 1)

        h_ij_2 = self.linear2_nb_2(
            nn.swish(self.linear1_nb_2(z_ij))
        )  # (N, N, hiddenFeatures)

        h_i_2 = jnp.sum(
            jnp.where(~eye_mask, g_ij_2 * h_ij_2, 0.0), axis=1
        ) / N  # (N, hiddenFeatures)

        jastrow_nb_2 = jnp.sum(
            self.jastrow2_nb_2(nn.swish(self.jastrow1_nb_2(h_i_2)))
        ) / N

        backflow_nb_2 = self.backflow2_nb_2(
            nn.swish(self.backflow1_nb_2(h_i_2))
        )  # (N, dim)

        xs = rs + backflow_nb + backflow_nb_2

        CYJastrow = self.CYJastrow(rs)
        slaterUp = self.slaterUp(xs[:self.spins[0], :])
        slaterDown = self.slaterDown(xs[self.spins[0]:, :])

        return slaterUp + slaterDown + CYJastrow + jastrow_nb + jastrow_nb_2
...
wavefunction = TestWavefunction(
    spins=spins, dim=dim, lattice=lattice, kpoints=kpoints, hiddenFeatures=32
)
...
referenceParameters = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/width/one32/rs{}_best/parameters.msgpack".format(str(r_ws))
)
