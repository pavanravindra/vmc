...
upCoeffs = jnp.eye(numKpoints, spins[0]) + parameter_noise * np.random.normal(size=coeffShape)
downCoeffs = jnp.eye(numKpoints, spins[1]) + parameter_noise * np.random.normal(size=coeffShape)
