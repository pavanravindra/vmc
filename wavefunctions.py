...
class LogTwoBodySJBDirectional(Wavefunction):
    """
    Slater-Jastrow wavefunction with following specs:
    - Slater: RHF ground state
    - Jastrow: Coulomb-Yukawa + neural function acting on pairwise information
        NOTE: This basically comes up with the "best" two-body Jastrow.
    - Backflow: acts purely on pairwise information to produce a backflow
                acting in the direction of r_{ij}
    """
    spins : (int,int)
    L : float
    hiddenFeatures : int
    ...
    def setup(self):
        ...
        self.neuralBackflow1 = nn.Dense(self.hiddenFeatures)
        self.neuralBackflow2 = nn.Dense(1)
    ...
    def __call__(self, rs):
        ...
        backflow = jnp.sum(
            decays * disps * self.neuralBackflow2(nn.swish(
                self.neuralBackflow1(v_ij)
            )), axis=1
        )        
        xs = rs + backflow
