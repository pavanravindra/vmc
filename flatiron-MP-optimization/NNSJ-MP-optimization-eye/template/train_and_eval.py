...
class LogMPNNSJ(wavefunctions.Wavefunction):
    """
...
diagMatrix = jnp.eye(numKpoints, spins[0])
upCoeffs = diagMatrix # + 1e-6 * np.random.normal(size=(numKpts, spins[0]))
downCoeffs = diagMatrix # + 1e-6 * np.random.normal(size=(numKpts, spins[0]))
