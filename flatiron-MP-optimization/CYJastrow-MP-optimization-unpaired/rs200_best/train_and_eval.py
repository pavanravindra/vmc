...
class LogMPFixedCYJastrow(wavefunctions.Wavefunction):
    """
    SJ wavefunction. The reference state is specified and fixed at
    initialization. The Jastrow is of the Coulomb-Yukawa form.

    Since the reference state is fixed at initialization, there are still only
    two parameters, both in the CY Jastrow.
    """
    spins : int
    dim : int
    lattice : jnp.ndarray
    ...
    def setup(self):
        ...
        self.slaterDown = wavefunctions.LogMPSlater(
            self.spins[0], self.dim, self.kpoints, self.downCoeffs
        )
        self.CYJastrow = wavefunctions.LogFixedCYJastrow(
            self.spins, self.lattice, As
        )
...
diagMatrix = np.eye(numKpoints, spins[0])
upCoeffs = jnp.array(diagMatrix)
diagMatrix[spins[0]-1,spins[0]-1] = 0
diagMatrix[spins[0],spins[0]-1] = 1
downCoeffs = jnp.array(diagMatrix)
...
As = wavefunctions.loadParameters(
    "../../CYJastrow/rs{}_best/parameters.msgpack".format(r_ws)
)['params']['CYJastrow']['As_same_diff']
print(As)

wavefunction = LogMPFixedCYJastrow(
    spins, dim, lattice, kpoints, upCoeffs, downCoeffs, As
)
...
print("NUM PARAMETERS:\t{}".format(optimization.flatten(parameters).size))
