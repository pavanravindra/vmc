...
def occ_columns_from_theta(theta: jnp.ndarray) -> jnp.ndarray:
    M_N, N = theta.shape          # (Nk - N_spin, N_spin)
    M = M_N + N
    # Block anti-Hermitian matrix
    A = jnp.zeros((M, M), dtype=theta.dtype)
    A = A.at[N:, :N].set(theta)
    A = A.at[:N, N:].set(-theta.T)
    # Matrix exponential — smooth gradient everywhere, including at A=0
    return jax.scipy.linalg.expm(A)[:, :N]
