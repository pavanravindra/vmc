...
class TestWavefunction(wavefunctions.Wavefunction):
    ...
    def setup(self):
        ...
        self.newLinear1 = nn.Dense(self.hiddenFeatures)
        self.newLinear2 = nn.Dense(
    ...
    def __call__(self, rs):
        ...
        h_i = jnp.sum(
            jnp.where(~eye_mask, h_ij, 0.0), axis=1
        ) / N                                                   # (N,d)
        ...
        n_ij = self.newLinear2(nn.swish(self.newLinear1(v_ij))) # (N,N,dim+1)
        jastrowFeatures = n_ij[:,:,0]                           # (N,N)
        backflowFeatures = n_ij[:,:,1:]                         # (N,N,dim)

        newBackflow = jnp.average(backflowFeatures, axis=1)     # (N,dim)
        newJastrow = 0.5 * jnp.sum(jastrowFeatures) / sum(self.spins)

        xs = rs + backflow + newBackflow
        
        slaterUp = self.slaterUp(xs[:self.spins[0],:])
        slaterDown = self.slaterDown(xs[self.spins[0]:,:])
        
        return slaterUp + slaterDown + CYJastrow + neuralJastrow + newJastrow
