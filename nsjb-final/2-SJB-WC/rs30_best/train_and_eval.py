...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(1e-1,1e2)
...
class TestWavefunction(wavefunctions.Wavefunction):
    ...
    def setup(self):
        ...
        self.slaterUp = wavefunctions.LogFixedMPSlater(self.spins[0], self.dim, self.kpoints, upCoeffs)
        self.slaterDown = wavefunctions.LogFixedMPSlater(self.spins[0], self.dim, self.kpoints, downCoeffs)
        self.CYJastrow = wavefunctions.LogCYJastrow(self.spins, self.lattice)

        self.linear1_tb = nn.Dense(self.hiddenFeatures)
        self.linear2_tb = nn.Dense(
    ...
    def __call__(self, rs):
        ...
        n_ij = self.linear2_tb(nn.swish(self.linear1_tb(v_ij))) # (N,N,dim+1)
        m_ij = jnp.where(~eye_mask, n_ij, 0.0)                  # (N,N,dim+1)

        jastrowFeatures_tb = m_ij[:,:,0]                        # (N,N)
        backflowFeatures_tb = m_ij[:,:,1:]                      # (N,N,dim)

        jastrow_tb = 0.5 * jnp.sum(jastrowFeatures_tb) / N
        backflow_tb = jnp.sum(backflowFeatures_tb, axis=1) / N
        ...
        xs = rs + backflow_tb # + backflow_nb
        
        CYJastrow = self.CYJastrow(rs)
        slaterUp = self.slaterUp(xs[:self.spins[0],:])
        slaterDown = self.slaterDown(xs[self.spins[0]:,:])
        
        return slaterUp + slaterDown + CYJastrow + jastrow_tb # + jastrow_nb

upCoeffs = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/ortho-test/CYJastrow-MP-WC-random/rs{}_best/parameters.msgpack".format(str(r_ws))
)['params']['slaterUp']['MP_coefficients']
downCoeffs = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/ortho-test/CYJastrow-MP-WC-random/rs{}_best/parameters.msgpack".format(str(r_ws))
)['params']['slaterDown']['MP_coefficients']

wavefunction = TestWavefunction(spins, dim, lattice, kpoints, 32, upCoeffs, downCoeffs)
