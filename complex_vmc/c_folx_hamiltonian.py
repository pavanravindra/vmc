...
class LocalKineticEnergy(LocalEnergy):
    ...
    We work with log-wavefunctions, so we have Psi(R) = exp(f(R)).

    f(R) = a + ib where a = log|Psi(R)| and b is the angle of the phase.

    The local kinetic energy is T = -0.5 * sum_i [ grad_i^2 f + (grad_i f)^2 ].
    You can verify this by starting from T = -0.5 * sum_i [ grad_i^2 Psi / Psi ].

    The first term is the Laplacian term and the second term is the Jacobian term.

    You then have to apply the Laplacian and Jacobian operations to the a and b
    terms separately, including the factor of i in front of everything coming from
    the b term.

    T = -0.5 * sum_i {
        (grad^2 a + i grad^2 b) +
        ((grad a)^2 - (grad b)^2 + 2i grad a \cdot grad b)
    }
    """
    ...
    def configuration(self, parameters, rs):
        ...
        laplacian_a = folx_output[1].laplacian
        laplacian_b = folx_output[0].laplacian
        jacobian_a = folx_output[1][1].dense_array
        jacobian_b = folx_output[0][1].dense_array
        
        grad_a2 = jnp.sum(jacobian_a ** 2)
        grad_b2 = jnp.sum(jacobian_b ** 2)
        grad_mix = jnp.sum(jacobian_a * jacobian_b)

        laplacianContribution = laplacian_a + 1j * laplacian_b
        jacobianContribution = grad_a2 - grad_b2 + 2j * grad_mix
        kineticEnergy = -0.5 * (laplacianContribution + jacobianContribution)
