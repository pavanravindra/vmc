...
class StochasticReconfigurationMomentum:
    ...
    def __init__(self, logWavefunction, localEnergy, mode='normal'):
        self.logWavefunction = logWavefunction
        self.localEnergy = localEnergy
        self.mode = mode
        self.parameterGradBatchFlat = jax.vmap(
            self.getParameterGradient, in_axes=(None,0)
    ...
    def __call__(self, parameters, walkerRs, learningRate, diagonalShift, mu, history):
        ...
        median = jnp.median(jnp.real(localEnergies))
        mad = jnp.median(jnp.abs(jnp.real(localEnergies) - median))
        localEnergies = jnp.clip(
            jnp.real(localEnergies), median - 5*mad, median + 5*mad
        ).astype(localEnergies.dtype)
        ...
        if self.mode == 'normal':
            s_jk = jnp.real(U.T.conj() @ U / numWalkers)
    ...
    def getParameterGradient(self, parameters, r):
        """
        This function should return a flattened array of the parameter
        gradients for the single configuration specified.
        """

        def getPhase(parameters, r):
            return self.logWavefunction.apply(parameters, r)[0]
        def getLogmag(parameters, r):
            return self.logWavefunction.apply(parameters, r)[1]

        def getPhaseGrad(parameters, r):
            return jax.grad(getPhase, argnums=0)(parameters, r)
        def getLogmagGrad(parameters, r):
            return jax.grad(getLogmag, argnums=0)(parameters, r)

        phaseGrad = getPhaseGrad(parameters, r)
        logmagGrad = getLogmagGrad(parameters, r)

        phaseGradFlat = jax.flatten_util.ravel_pytree(phaseGrad)[0]
        logmagGradFlat = jax.flatten_util.ravel_pytree(logmagGrad)[0]
        
        max_phase_norm = 1.0  # tune this
        phase_norm = jnp.linalg.norm(phaseGradFlat)
        phaseGradFlat = phaseGradFlat * jnp.minimum(1.0, max_phase_norm / (phase_norm + 1e-8))

        return logmagGradFlat + 1j * phaseGradFlat
