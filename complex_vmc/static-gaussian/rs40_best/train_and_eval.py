...
class TestWavefunction(wavefunctions.Wavefunction):
    ...
    lattice : jnp.ndarray
    kpoints : jnp.ndarray
    upCenters : jnp.ndarray
    downCenters : jnp.ndarray

    def setup(self):
        
        self.slaterUp = wavefunctions.LogGaussianSlater(
            self.spins[0], self.dim, self.r_ws, self.kpoints, self.upCenters
        )
        self.slaterDown = wavefunctions.LogGaussianSlater(
            self.spins[1], self.dim, self.r_ws, self.kpoints, self.downCenters
        )
    ...
    def __call__(self, rs):
        ...
        logmag = logmagUp + logmagDown + CYJastrow
...
wavefunction = TestWavefunction(
    spins, dim, r_ws, lattice, kpoints, upCenters, downCenters
)
