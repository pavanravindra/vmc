...
class MetropolisUpdater:
    ...
    def __init__(self, logWavefunction):
        self.logmag = lambda parameters, rs: logWavefunction.apply(parameters, rs)[1]
    ...
    def updateConfiguration(self, parameters, rs1, rng, tau):
        ...
        probRatio = jnp.exp(
            2 * (self.logmag(parameters, rs2) - self.logmag(parameters, rs1))
        )
...
class MALAUpdater:
    ...
    def __init__(self, logWavefunction, r_ws, clipGradients=True):
        self.logmag = lambda parameters, rs: logWavefunction.apply(parameters, rs)[1]
        self.gradLogmag = jax.grad(self.logmag, argnums=1)
        self.r_ws = r_ws
    ...
    def updateConfiguration(self, parameters, rs1, rng, tau):
        ...
        def proposalProb(Ri, Rf):
            ...
            localGrad = self.gradLogmag(parameters,Ri)
        ...
        localGrad = self.gradLogmag(parameters, rs1)
        gradShift = localGrad * tau
        if self.clipGradients:
            gradShift = clip(gradShift)
