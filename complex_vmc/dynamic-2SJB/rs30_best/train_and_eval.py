...
sys.path.append("/burg-archive/ccce/users/phr2114/vmc/complex_vmc")
...
import c_wavefunctions as wavefunctions
import c_folx_hamiltonian as folx_hamiltonian
import c_trajectory as trajectory
import c_optimization as optimization
...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(1e-2,1e2)
...
class TestWavefunction(wavefunctions.Wavefunction):
    ...
    def __call__(self, rs):
        ...
        h_jastrow = jnp.where(                                  # (N,N,1)
            ~eye_mask,
            self.jastrow2_nb(nn.swish(self.jastrow1_nb(v_ij))),
            0.0
        )
        jastrow_nb = 0.5 * jnp.sum(h_jastrow) / N

        h_backflow = jnp.where(                                  # (N,N,dim)
            ~eye_mask,
            self.backflow2_nb(nn.swish(self.backflow1_nb(v_ij))),
            0.0
        )
        backflow_nb = jnp.sum(h_backflow, axis=1) / N

        xs = rs + backflow_nb
        
        CYJastrow = self.CYJastrow(rs)
        angleUp, logmagUp = self.slaterUp(xs[:self.spins[0],:])
        angleDown, logmagDown = self.slaterDown(xs[self.spins[0]:,:])

        angle = angleUp + angleDown
        logmag = logmagUp + logmagDown + CYJastrow + jastrow_nb
...
tunedAs = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/complex_vmc/CYJastrow/rs{}_best/parameters.msgpack".format(str(r_ws))
