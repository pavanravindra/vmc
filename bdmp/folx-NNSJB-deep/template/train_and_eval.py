...
class LogTwoBodySJB(wavefunctions.Wavefunction):
    """
    Neural 2-body SJB wavefunction with 2 hidden layers (instead of 1).
    """
    spins : (int,int)
    dim : int
    lattice : jnp.ndarray
    ...
    def __call__(self, rs):
        ...
        n_ij = self.linear3(nn.swish(self.linear2(nn.swish(self.linear1(
            v_ij
        )))))       # (N,N,dim+1)
...
wavefunction = LogTwoBodySJB(spins, dim, lattice, kpoints, 64)
