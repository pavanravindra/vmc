...
class LocalMP(wavefunctions.Wavefunction):
    ...
    def setup(self):
        ...
        self.Wqs = nn.Dense(self.d, use_bias=False)
        self.Wks = nn.Dense(self.d, use_bias=False)

        self.ws = self.param('w0', lambda _ : jnp.zeros(self.d))
        self.MLP_att0 = nn.Dense(self.d)
        self.MLP_att1 = nn.Dense(self.d)

        self.MLP_msg0 = nn.Dense(self.d)
        self.MLP_msg1 = nn.Dense(self.d)
        
        self.MLP_node0 = nn.Dense(self.d)
        self.MLP_node1 = nn.Dense(self.d)
    ...
    def __call__(self, rs):
        ...
        qit = self.Wqs(hit)
        kit = self.Wks(hit)
        sijt = qit @ kit.T / jnp.sqrt(self.d) + self.MLP_att1(nn.swish(self.MLP_att0(hijt))) @ self.ws
        gijt = nn.sigmoid(sijt)

        mijt = self.MLP_msg1(nn.swish(self.MLP_msg0(hijt)))
        eye_mask = jnp.eye(N, dtype=bool)[:,:,None]
        Mit = jnp.sum(
            jnp.where(~eye_mask, gijt[:,:,None] * mijt, 0.0), axis=1
        )

        hit = hit + self.MLP_node1(nn.swish(self.MLP_node0(
...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(1e-2,2e1)
...
wavefunction = LocalMP(spins, dim, lattice, kpoints, 32)
...
tunedAs = wavefunctions.loadParameters(
    "../../../dynamic-test/CYJastrow/rs{}_best/parameters.msgpack".format(str(r_ws))
