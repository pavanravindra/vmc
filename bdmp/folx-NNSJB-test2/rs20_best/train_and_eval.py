...
class TestWavefunction(wavefunctions.Wavefunction):
    ...
    def setup(self):
        ...
        self.jastrowWeights1 = nn.Dense(self.hiddenFeatures)
        self.jastrowWeights2 = nn.Dense(1)

        self.backflowWeights1 = nn.Dense(self.hiddenFeatures)
        self.backflowWeights2 = nn.Dense(1)
    ...
    def __call__(self, rs):
        ...
        jastrowWeights = nn.sigmoid(
            self.jastrowWeights2(nn.swish(self.jastrowWeights1(v_ij)))
        )                                                            # (N,N,1)
        jastrowValues = self.jastrow2(nn.swish(self.jastrow1(v_ij))) # (N,N,1)
        neuralJastrow = 0.5 * jnp.sum(
            jnp.where(~eye_mask, jastrowWeights * jastrowValues, 0.0)
        ) / N

        backflowWeights = nn.sigmoid(
            self.backflowWeights2(nn.swish(self.backflowWeights1(v_ij)))
        )                                                            # (N,N,1)
        backflowValues = self.backflow2(nn.swish(self.backflow1(v_ij))) # (N,N,dim)
        backflow = jnp.sum(
            jnp.where(~eye_mask, backflowWeights * backflowValues, 0.0), axis=1
        ) / N
