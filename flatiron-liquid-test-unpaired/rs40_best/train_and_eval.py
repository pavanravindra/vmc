...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(5e-1,1e2)
...
up_rng, down_rng = jax.random.split(rng, 2)
diagonalMatrix = np.eye(numKpts, spins[0])
spinUpCoeff = jnp.array(diagonalMatrix) + 5e-2 * jax.random.normal(up_rng, shape=(numKpts, spins[0]))
diagonalMatrix[spins[0]-1,spins[0]-1] = 0
diagonalMatrix[spins[0],spins[0]-1] = 1
spinDownCoeff = jnp.array(diagonalMatrix) + 5e-2 * jax.random.normal(down_rng, shape=(numKpts, spins[1]))

np.save("init_spinUpCoeff.npy", spinUpCoeff)
np.save("init_spinDownCoeff.npy", spinDownCoeff)

wavefunction = wavefunctions.LogFixedMPTwoBodySJB(
    spins, dim, lattice, kpoints, spinUpCoeff, spinDownCoeff, 32
)
