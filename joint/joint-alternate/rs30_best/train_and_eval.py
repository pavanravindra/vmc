...
import wavefunctions, folx_hamiltonian, trajectory
import local_optimization as optimization
...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(5e-1,3e1)
    hyperparameters[1] = optimization.logSample(1e0,1e3)
...
dynamicEta = hyperparameters[0]
staticEta = hyperparameters[1]
...
###############################################################################
#   Creating parameter masks for alternating optimization                     #
###############################################################################
...
allParametersMask = optimization.castFloatAsPytree(1.0, parameters)

dynamicParametersMask = optimization.castFloatAsPytree(1.0, parameters)
for path in staticParametersPaths:
    dynamicParametersMask = optimization.replaceSubtreeLeaves(dynamicParametersMask, path, 0.0)

staticParameterMask = optimization.castFloatAsPytree(0.0, parameters)
for path in staticParametersPaths:
    staticParameterMask = optimization.replaceSubtreeLeaves(staticParameterMask, path, 1.0)
for path in cyParametersPaths:
    staticParameterMask = optimization.replaceSubtreeLeaves(staticParameterMask, path, 1.0)

print("PARAMETER MASK INFO:")

print(np.sum(optimization.flatten(allParametersMask)))
print(optimization.flatten(allParametersMask).shape)

print(np.sum(optimization.flatten(dynamicParametersMask)))
print(optimization.flatten(dynamicParametersMask).shape)

print(np.sum(optimization.flatten(staticParameterMask)))
print(optimization.flatten(staticParameterMask).shape)
...
history = jnp.zeros_like(optimization.flatten(parameters))
...
for dt in range(trainSteps):
    ...
    if maskType == "dynamic":
        parameterMask = dynamicParametersMask
        eta = dynamicEta
    elif maskType == "static":
        parameterMask = staticParameterMask
        eta = staticEta
    else:
        raise Exception("Invalid mask type specified.")

    ( maxNorm , currentEnergies , newParameters , history ) = updateParameters(
        parameters, rs, eta, diagonalShift, mu, history, parameterMask
    )
    ...
    print("Step {:5d}   Energy: {:.5f}   Norm: {:5}   Min Acc: {:.3f}   Avg Acc: {:.3f}   Param Norm: {:.5f}   Mask: {}".format(
        dt, currentEnergy, str(maxNorm),
        np.min(acceptArrays), np.average(acceptArrays),
        np.linalg.norm(optimization.flatten(parameters)), maskType
    ))

    if (dt + 1) % switchMaskEvery == 0:
        history = jnp.zeros_like(optimization.flatten(parameters))
        if maskType == "dynamic":
            maskType = "static"
        elif maskType == "static":
