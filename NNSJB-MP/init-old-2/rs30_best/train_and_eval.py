...
if not os.path.exists("hyperparameters.txt"):
    ...
    hyperparameters[0] = optimization.logSample(1e-2,5e2)
...
tuned = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/ortho-test/CYJastrow-MP-WC-random/rs{}_best/parameters.msgpack".format(str(r_ws))
)
parameters['params']['slaterUp'] = tuned['params']['slaterUp']
parameters['params']['slaterDown'] = tuned['params']['slaterDown']

tuned = wavefunctions.loadParameters(
    "/burg-archive/ccce/users/phr2114/vmc/dynamic-test/NNSJB/rs{}_best/parameters.msgpack".format(str(r_ws))
)
parameters['params']['CYJastrow'] = tuned['params']['CYJastrow']
parameters['params']['linear1'] = tuned['params']['linear1']
parameters['params']['linear2'] = tuned['params']['linear2']
...
for dt in range(trainSteps):
    ...
    if jnp.abs(jnp.average(currentEnergies)) / N > 1e4:
        raise Exception("Energy has diverged...")
